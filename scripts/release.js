...
const JAVA_ROOT = path.join(__dirname, "..", "dcp-java");
const PARENT_POM = path.join(JAVA_ROOT, "pom.xml");
const MODULE_POMS = [
  path.join(JAVA_ROOT, "dcp-core", "pom.xml"),
  path.join(JAVA_ROOT, "dcp-spring", "pom.xml"),
  path.join(JAVA_ROOT, "dcp-spring-boot-starter", "pom.xml"),
];
...
function getCurrentVersion() {
  ...
  if (!versionMatch) {
    throw new Error("Could not find project version in dcp-java/pom.xml");
  }
...
function main() {
  ...
  if (!versionType || !["patch", "minor", "major"].includes(versionType)) {
    ...
    log("\nThis will:", "yellow");
    log("  • Update all dcp-java pom.xml files to the release version", "cyan");
